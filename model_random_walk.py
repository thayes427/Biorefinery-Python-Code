...
def upload(filename, ind_col, dep_col='', date_col=''):
    data = np.loadtxt(filename,dtype = np.str, delimiter = ",",unpack=True,\
        skiprows = 1)
...
def random_walk(sigma,num_steps):
    mu = 0
    walk = []
    for step in range(num_steps):
        new_mu = np.random.normal(mu,sigma)
        walk.append(new_mu)
        mu = new_mu
    
    return walk

def add_randomness(sigma, model,start_range):
    '''
    Add a random walk to the data
    
    Inputs:
        best_abt: model generated tuple of the best (a,b,t) parameters
        ind, dep, dates 
    Output: model generated data with added degree of randomness
        the output data is shifted by the correct amount 
        dates are not shifted
    '''
    
    low,high = start_range
    fluctuations = np.asarray(random_walk(sigma,len(model)))
    start = random.randint(low,high)
    fluctuations -= fluctuations[start]
    model += fluctuations
...
def get_summary_stats(best_abt, ind, dep, conf = 95):
    ...
    Summary stats: mean of the residuals, standard of error on the residuals,
...
def predict(abt, ind, mu, std, vol = 0,span = 'month',cutoff = None, plot=False, randomness = True,ind_dep = "oil_ma"):
    '''
    ...
    INPUTS: 
        dates: the number of points one would want to generate
        ...
        vol: float greater than 0 and less than 1, prediction for increase in
            volatility
        abt: currently inout from best model, but could build in presets
        ind: independent data
        dep: historical dependent data but could be skipped and use only abt
    '''
    ind = gen_linspace(ind,span,cutoff)
    ...
    if randomness:
        ...
        model = add_randomness(sigma,model,start_range)
    if plot:
        plt.plot(ind)
        plt.plot(model)
        plt.xlabel('Years')
...
def gen_linspace(ind,span = 'month', cutoff= None):
    ...
    if span == 'year':
        ...
        for i,val in enumerate(ind[:-1]):
            months = np.linspace(ind[i],ind[i+1],13)
...
def overall_func(filename,ind_col, dep_col='', date_col='',a_min = .5,a_max = 1.7,\
                    b_min = 660,b_max = 680,n = 200,t_min = -3,t_max = 3,\
    ...
    if len(data) == 3:
        ...
        while (not range_correct) and range_search_iter < 1000:
            ...
            range_correct, a_s, b_s = range_moving(best_abt, a_s, b_s,\
                n, best_abt_lsqr)
            print("stuck in range search: "+ str(range_search_iter))
            range_search_iter += 1
        ...
        while (not range_precise) and range_narrowing_iter < 1000:
            ...
            print("stuck in narrowing: "+ str(range_narrowing_iter))
...
def single_point_predict(value,mu,std,vol,CAGR,num_months,plot= False):
    ...
    for month in time_span:
        ...
        #print(point_std)
        resid = np.random.normal(mu, point_std)
