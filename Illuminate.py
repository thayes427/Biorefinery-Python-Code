...
class MainApp(Tk):
    ...
    def create_simulation_object(self, simulation_vars, vars_to_change,
        ...
        new_sim = Simulation(
                ...
                self.bkp_ref, self.simulation_dist, save_freq=2, num_processes=self.num_processes)
...
class Simulation(object):
    ...
    def __init__(self, sims_completed, tot_sim, simulation_vars, output_file, directory, 
                 ...
                 fortran_dist_dict, save_freq=2, num_processes=1):
        ...
        self.fortran_dist_dict = self.manager.dict(fortran_dist_dict)
    ...
    def start_simulation(self, tasks):
        ...
        for i in range(self.num_processes):
            process_args = (self.current_COMS_pids, self.pids_to_ignore, self.aspenlock, 
                            ...
                            self.warning_keywords, self.bkp_ref, self.fortran_dist_dict)
...
def multiprocessing_worker(current_COMS_pids, pids_to_ignore, aspenlock, excellock, aspenfilename, save_bkps,
           ...
           warning_keywords, bkp_ref, fortran_dist_dict):
    '''
    ...
    for trial_num in iter(task_queue.get, 'STOP'):
        ...
        aspencom, case_values, run_summary, aspen_tree = aspen_run(
                ...
                directory, warning_keywords, fortran_dist_dict) 
...
def aspen_run(aspencom, aspen_tree, simulation_vars, trial, vars_to_change, directory, 
              warning_keywords, fortran_dist_dict):
    '''
    ...
    for (aspen_variable, aspen_call, fortran_index), dist in simulation_vars.items():
        ...
        if type(dist[trial]) == str:
            # if fortran variable
            variable_values[aspen_variable] = fortran_dist_dict[aspen_variable][trial]
        else:
