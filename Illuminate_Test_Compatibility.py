        ...
        if row['Toggle']: 
            try:
                if obj.FindNode(row['Variable Aspen Call']).Value is None:     
                    status_queue.put((True, 1, 'The value at the node "'+ row['Variable Aspen Call'] + \
                                      '" for variable "' + row['Variable Name'] + \
                                      '" is None. Are you sure this is the right path?'))
            except:
                status_queue.put((True, 1, 'Aspen call "'+ row['Variable Aspen Call'] + \
                                  '" for variable "' + row['Variable Name'] + \
    ...
    for index, row in df.iterrows():
        if row['Toggle'] and row['Fortran Call']:
            if row['Fortran Value to Change'] not in row['Fortran Call']:
                status_queue.put((True, 1, 'The fortran value to change "' + \
...
def test_calculator_file(calculator_file, aspen_file, status_queue):
    ...
    if output_tab_exists:
        if any(str(v) != "Variable Name" for v in book.Sheets('Output').Evaluate('B2')):
            status_queue.put((True,2,'Output tab is not configured properly. The column header for '+\
                              '"Variable Name"\nshould be in B3 so that the first variable name is in B4'))
        elif any(str(v) != "Value" for v in book.Sheets('Output').Evaluate('C2')):
            status_queue.put((True,2,'Output tab is not configured properly. The column header '+\
    ...
    try:
        ...
        status_queue.put((True,1,'"Set-up" tab missing from Excel calculator .xlsm file. '+\
    ...
    try:
        ...
        if not (file_extension=='.bkp' or file_extension == '.apw'):
            status_queue.put((True,2,'In the "Set-up" tab, the name of the .apw or .bkp '+\
                              'should be in cell B1. If the location of this\nreference needs '+\
                              'to be changed, make sure that you also change it in the "sub_GetSumData" macro'))
    except:
        setup_tab_functional = False
        status_queue.put((True,2,'In the "Set-up" tab, the name of the .apw or .bkp should '+\
                          'be in cell B1. If the location of this\nreference needs to be changed,'+\
    ...
    try:
        ...
        status_queue.put((True,1,'Macro with name "sub_ClearSumData_ASPEN" does '+\
                          'not exist in the Excel calculator .xlsm file'))
    
    try:
        if not all(str(v)=='None' for v in book.Sheets('aspen').Evaluate('C7:C20')):
            status_queue.put((True,2,'Excel macro sub_ClearSumData_ASPEN does not appear to '+\
                              'be working. Values in column C of sheet\n"aspen" are not being cleared.'))
    except:
        pass
    ...
    if setup_tab_functional:
        try:
            ...
            if not all(str(v)!='None' for v in book.Sheets('aspen').Evaluate('C8')):
                status_queue.put((True,2,'"sub_GetSumData_ASPEN" does not appear to be '+\
                                  'working. Values should be populated in column C of sheet "aspen"'))
                
        except:
            status_queue.put((True,2,'Macro with name "sub_GetSumData_ASPEN" does not '+\
                              'exist in the Excel calculator .xlsm file or calls\nanother'+\
